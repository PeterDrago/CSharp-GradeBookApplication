...
namespace GradeBook.GradeBooks
{
    public class RankedGradeBook : BaseGradeBook
    {
        ...
        public override void CalculateStudentStatistics(string name)
        {
            if (Students.Count < 5)
            {
                Console.WriteLine("Ranked-grading requires at least 5 students with grades in order to properly calculate a studen'ts overall grade.");
            }
            else
            {
                return base.CalculateStudentStatistics();
            }
        }
    }
}
